...
# Copied from https://github.com/NNPDF/pineappl/blob/master/install-capi.sh
...
url="https://github.com/Radonirinaunimi/neopdf/releases/download/v${version}/neopdf_capi-${target}.tar.gz"
...
sed "s:prefix=/:prefix=${prefix}:" "${prefix}"/lib/pkgconfig/neopdf_capi.pc > \
    "${prefix}"/lib/pkgconfig/neopdf_capi.pc.new
mv "${prefix}"/lib/pkgconfig/neopdf_capi.pc.new "${prefix}"/lib/pkgconfig/neopdf_capi.pc
...
if "${pcbin}" --exists neopdf_capi; then
    found_prefix=$(cd "$("${pcbin}" --variable=prefix neopdf_capi)" && pwd)

    if [ "${prefix}" != "${found_prefix}" ]; then
        echo
        echo "Warning: Your PKG_CONFIG_PATH environment variable isn't properly set."
        echo "It appears a different installation of NeoPDF is found:"
        echo
        echo "  ${found_prefix}"
        echo
